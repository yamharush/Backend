...
async function getUserInfo(req: Request, res: Response) {
    try {
        ...
        try {
            ...
            res.status(400).send({ error: 'fail to get current user from db' });
        }
    } catch (err) {
        console.log({ err });
...
const updateUser = async (req: Request, res: Response) => {
    ...
    if (!updatedUser) {
        return sendError(res, 'please provide User');
    }
    const userToSave = {
    ...
    try {
        await User.replaceOne({ email: userToSave.email }, userToSave);
